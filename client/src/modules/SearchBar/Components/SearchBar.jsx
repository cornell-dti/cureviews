    ...
    if (this.state.query !== '' && !this.state.selected) {
      ...
      if (this.state.index === 0 && this.state.enter === 1) {
        ...
        return (
          <Redirect
            push
            to={`/results/keyword/${this.state.query.split(' ').join('+')}`}
          ></Redirect>
        )
      }
      ...
      let exact_search = (
        <a
          key={'search'}
          className={
            this.state.index === 0 && this.state.mouse !== 1
      ...
      let subjectList = this.state.allSubjects.slice(0, 3).map((subject, i) => (
        ...
        <SubjectResult
          ...
          query={this.state.query}
          active={
            this.state.index === i + 1 /* plus 1 because of exact search */
          }
      ...
      let professorList = this.state.allProfessors
        .slice(0, 3)
        .map((professor, i) => (
          //create a new class "button" that will set the selected class to this class when it is clicked.
          <ProfessorResult
      ...
      results.push(
        this.state.allCourses.slice(0, 5).map((course, i) => (
          //create a new class "button" that will set the selected class to this class when it is clicked.
          <Course
            key={course._id}
            info={course}
            query={this.state.query}
            handler={this.setCourse}
            active={
              this.state.index ===
              i +
                subjectList.length +
      ...
      >
        ...
        >
          <input
            ...
            defaultValue={
              this.props.isInNavbar
                ? this.props.userInput
                  ? this.props.userInput
                  : ''
                : ''
            }
            placeholder={this.props.isInNavbar ? '' : text}
            autoComplete="off"
          />
