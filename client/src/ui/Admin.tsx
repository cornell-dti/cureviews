...
export default function Admin() {
    ...
    useEffect(() => {
        async function confirmAdmin() {
            const res = await axios.post(`/v2/tokenIsAdmin`, { token: token });
    ...
    function approveReview(review: Review) {
        axios.post("/v2/makeReviewVisible", { review: review, token: token })
            .then((response) => {
                const result = response.data.result;
                if (result.resCode === 1) {
    ...
    function removeReview(review: Review, isUnapproved: boolean) {
        axios.post("/v2/removeReview", { review: review, token: token })
            .then((response) => {
    ...
    function unReportReview(review: Review) {
        axios.post("/v2/undoReportReview", { review: review, token: token })
            .then((response) => {
                const result = response.data.result.resCode;
                if (result === 1) {
    ...
    function updateProfessors() {
        ...
        axios.post("/v2/setProfessors", { token: token })
            .then((response) => {
                const result = response.data.result.resCode;
                if (result === 0) {
    ...
    function resetProfessors() {
        ...
        axios.post("/v2/resetProfessors", { token: token })
            .then((response) => {
                const result = response.data.result.resCode;
                if (result === 1) {
        ...
        if (doubleClick) {
            return (
                <div className="btn-group separate-buttons" role="group">
                    <button disabled={disableInit} type="button" className="btn btn-warning" onClick={() => addAllCourses()}>Initialize Database</button>
                </div>
            );
        } else {
            ...
            return (
                <div className="btn-group separate-buttons" role="group">
                    <button type="button" className="btn btn-warning" onClick={() => firstClickHandler()}>Initialize Database</button>
                </div>
            );
        }
    }
                    ...
                    <div className="container-width whiteBg">
                        <div className="width-90">
                            ...
                            <div className="text-right">
                                <div className="btn-group separate-buttons" role="group">
                                    <button disabled={disableNewSem} type="button" className="btn btn-warning" onClick={() => addNewSem()}>Add New Semester</button>
                                </div>
                                <div className="btn-group separate-buttons" role="group">
                                    <button type="button" className="btn btn-warning" onClick={() => updateProfessors()}>Update Professors</button>
                                </div>
                                <div className="btn-group separate-buttons" role="group">
                                    <button type="button" className="btn btn-warning" onClick={() => resetProfessors()}>RESET Professors</button>
                                </div>
                                    ...
                                    <ul>
                                        {unapprovedReviews.map((review: Review) => {
                                            if (review.reported !== 1) {
                                                return <UpdateReview key={review._id} info={review} removeHandler={removeReview} approveHandler={approveReview} unReportHandler={approveReview} />;
                                            }
                                            return null;
                                        })
                                        }
                                ...
                                <div className="card-body">
                                    <ul>
                                        {unapprovedReviews.map((review: Review) => {
                                            ...
                                            if (review.reported === 1) {
                                                return <UpdateReview key={review._id} info={review} removeHandler={removeReview} approveHandler={approveReview} unReportHandler={unReportReview} />
                                            }
                                            return null;
                                        })}
                                    </ul>
