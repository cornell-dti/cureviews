...
adminRouter.post('/users/add', async (req, res) => {
  const { token, userId, role }: AdminUserRequestType = req.body;

  try {
    const auth = new Auth({ token });
    const result = await addAdmin({ auth: auth, id: userId, role });

    if (result) {
      return res.status(200).json({
        message: `Granted admin privilege to user with id ${userId} and role ${role}`
      });
    }
