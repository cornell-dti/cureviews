...
export const findAllPendingReviews = async () => await Reviews.find(
  { visible: 0 },
  {},
  { sort: { date: -1 }, limit: 700 },
).exec();

export const findAllReviewsAfterDate = async (date: Date) => await Reviews.aggregate([
  { $match: { date: { $gte: date } } },
  { $sample: { size: 1 } },
]);
