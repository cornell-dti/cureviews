...
const updateCourseWithAI = async (courseId: string) => {
  try {
    ...
    const reviewsText = courseReviews
    ...
    if (missingTags.length > 0) {
      console.log(
        `Missing tags for course ${courseId}: ${missingTags.join(', ')}`
      );
      return false;
    }
    console.log(
      `Course ${courseName} updated successfully with summary and tags.`
    );
    return true;
  } catch (error) {
    console.error(
      `Error updating course with summary and tags: ${error.message}`
    );
    return false;
  }
};
