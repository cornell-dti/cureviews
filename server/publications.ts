...
*/
Meteor.publish('classes', async function validClasses(searchString) {
  if (searchString !== undefined && searchString !== "") {
    ...
    if (indexFirstDigit == 0) {
      ...
      return await Classes.find(
        ...
        {}, { sort: {classFull: 1}, limit: 200, reactive: false }).exec();
    }
...
const searchWithinSubject = async (sub: string, remainder: string) => {
  return await Classes.find(
...
/* Publish a subset of the local database's Reviews collection based on the requested parameters.
   ...
   Else:
     return none
*/
Meteor.publish('reviews', async function validReviews(courseId: string, visiblity: 0 | 1, reportStatus: number, token: string) {
    let ret = null;
    let userIsAdmin;
...
*/
Meteor.publish('users', async function getUser(netId) {
    return await Students.find({ netId: netId }, {}, { limit: 20 }).exec();
});
 ...
 */
Meteor.publish('classInfo', async function(subject: string, number: string, api_key: string) {
  ...
  if (numberRegex.test(number) && subjectRegex.test(subject) && keyRegex.test(api_key)) {
    ...
    return await Classes.find(
          ...
          {}, { fields: { classSub: 1, classNum: 1, classDifficulty: 1, classRating: 1, classWorkload: 1 } }).exec();
  }
  else {
    return null;
  }
