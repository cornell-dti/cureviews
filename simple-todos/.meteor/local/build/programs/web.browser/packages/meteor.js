...
var withoutInvocation = function (f) {                                                                    // 1
  if (Package.ddp) {                                                                                      // 2
    var _CurrentMethodInvocation = Package.ddp.DDP._CurrentMethodInvocation;                              // 3
    if (_CurrentMethodInvocation.get() && _CurrentMethodInvocation.get().isSimulation)                    // 4
      throw new Error("Can't set timers inside simulations");                                             // 5
    return function () { _CurrentMethodInvocation.withValue(null, f); };                                  // 6
  }                                                                                                       // 7
  else                                                                                                    // 8
    return f;                                                                                             // 9
};                                                                                                        // 10
...
_.extend(Meteor, {                                                                                        // 16
  ...
  // should clear out the CurrentMethodInvocation environment variable.                                   // 19
                                                                                                          // 20
  /**                                                                                                     // 21
   * @memberOf Meteor                                                                                     // 22
